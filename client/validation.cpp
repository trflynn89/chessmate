...
int validation::validatePawn(Tile *temp)
{
    ...
    if(temp->pieceColor)
    {
        ...
        {
            expp[max++]=tile[row-1][col]->tileNum;
            retVal=1;
        }

        if(row==6 && !tile[5][col]->piece && !tile[4][col]->piece)
        {
            expp[max++]=tile[row-2][col]->tileNum;
            retVal=1;
        }
        ...
        if(row-1>=0 && col-1>=0)
        {
            ...
            {
            expp[max++]=tile[row-1][col-1]->tileNum;
            retVal=1;
            }
        }
...
void validation::orange()
{
    ...
    for(i=0;i<max;i++)
        tile[expp[i]/8][expp[i]%8]->setStyleSheet("QLabel {background-color: orange;}");
}
