...
public class Board
{
	...
	public Board()
	{
		...
		for(short i=0; i<Constants.NUM_RANKS; i++)
		{
			...
			{
				m_board[i][j] = new SquareGUI
				(
					i, j, SquareGUI.getSquareImage(i, j)
				);
			}
		}
		initializePieces();
		...
		if(move.getEndRank() == homeRank && move.getStartRank() == fromRank)
		{
			...
			if(s.getPiece() == pawn)
			{
			    ...
			    do
		    	{
		    		...
		    		(
		    			null, "Select a piece", "Pawn Promotion", 0,
	...
	private void setAmbiguityType(Move move)
	{
		...
		if
		(
			move.isCapture() && (movingPiece == Piece.WHITE_PAWN ||
     ...
     */
	private boolean spacesBetweenAreEmpty
	(
		int sRank, int sFile, int eRank, int eFile, Piece piece
	)
	{
